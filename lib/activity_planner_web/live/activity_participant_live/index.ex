  ...
  def render(assigns) do
    ~H"""
      ...
      <.table
        ...
        row_click={fn {_id, activity_participant} -> JS.navigate(~p"/activity_participants/#{activity_participant}") end}
      >
        <:col :let={{_id, activity_participant}} label="Activity"><%= activity_participant.activity_id %></:col>
        <:col :let={{_id, activity_participant}} label="Participant"><%= activity_participant.participant_id %></:col>
      ...
      <.modal :if={@live_action in [:new, :edit]} id="activity_participant-modal" show on_cancel={JS.patch(~p"/activity_participants")}>
        <.live_component
