defmodule ActivityPlannerWeb.NotificationTemplateLive.Index do
  ...
  def render(assigns) do
    ~H"""
      ...
      >
        <:col :let={{_id, notification_template}} label="Title"><%= notification_template.title %></:col>
        <:col :let={{_id, notification_template}} label="Content"><%= notification_template.template_content %></:col>
      ...
      <.modal :if={@live_action in [:new, :edit]} id="notification-template-modal" show on_cancel={JS.patch(~p"/notification_templates")}>
        <.live_component
