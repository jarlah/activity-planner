defmodule ActivityPlanner.JobManager do
  use GenServer

  alias ActivityPlanner.Notifications.NotificationSchedule

  # Client API

  def start_link(_) do
    GenServer.start_link(__MODULE__, :ok, name: {:global, :job_manager})
  end

  def add_job(%NotificationSchedule{} = schedule) do
    case GenServer.call({:global, :job_manager}, {:add, schedule}) do
      :ok -> {:ok, schedule}
      {:error, reason} -> {:error, reason}
      _ -> {:error, :unknown}
    end
  end

  def delete_job(%NotificationSchedule{} = schedule) do
    case GenServer.call({:global, :job_manager}, {:delete, schedule}) do
      :ok -> {:ok, schedule}
      {:error, reason} -> {:error, reason}
      _ -> {:error, :unknown}
    end
  end

  # Server Callbacks

  def init(:ok) do
    send(self(), :load_schedules)
    {:ok, nil}
  end

  def handle_info(:load_schedules, state) do
    IO.puts("Loading notification schedules from database")
    jobs = ActivityPlanner.Notifications.list_notification_schedules(skip_company_id: true)
    IO.puts("Found #{length(jobs) |> Integer.to_string()} notification schedules")
    Enum.each(jobs, &add_job_to_quantum/1)
    {:noreply, state}
  end

  def handle_call({:add, %NotificationSchedule{} = schedule}, _from, state) do
    case add_job_to_quantum(schedule) do
      {:ok, _} -> {:reply, :ok, state}
      {:error, reason} -> {:reply, {:error, reason}, state}
    end
  end

  def handle_call({:delete, %NotificationSchedule{} = schedule}, _from, state) do
    case delete_job_from_quantum(schedule) do
      {:ok, _} -> {:reply, :ok, state}
      {:error, reason} -> {:reply, {:error, reason}, state}
    end
  end

  defp add_job_to_quantum(%NotificationSchedule{} = schedule) do
    try do
      # this is necessary to avoid leakage.
      # Without it `[debug] Scheduling job for execution` is spammed the number of times any job has been added with add_job
      # see https://github.com/quantum-elixir/quantum-core/discussions/580
      # and https://elixirforum.com/t/programmatic-scheduling-of-quantum-jobs-leakage/58609
      :ok = ActivityPlanner.JobHelper.delete_quantum_job(schedule)
      :ok = ActivityPlanner.JobHelper.add_quantum_job(schedule)
      {:ok, "Quantum job added successfully"}
    rescue
      exception ->
        {:error, Exception.message(exception)}
    end
  end

  defp delete_job_from_quantum(%NotificationSchedule{} = schedule) do
    try do
      :ok = ActivityPlanner.JobHelper.delete_quantum_job(schedule)
      {:ok, "Quantum job deleted successfully"}
    rescue
      exception ->
        {:error, Exception.message(exception)}
    end
  end
end
